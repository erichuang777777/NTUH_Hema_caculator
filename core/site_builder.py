...
function openIssueReporter(drugId){const d=findDrugById(_HD,drugId);if(!d) return;const el=document.getElementById('issueC');el.innerHTML=`<div class=\"modal-header\"><div class=\"modal-icon\">&#9888;</div><div style=\"flex:1\"><div class=\"modal-title\">問題通報</div><div class=\"modal-subtitle\">${escHtml(d.generic_name)} &middot; ${escHtml(d.trade_names||'')}</div></div><button class=\"modal-close\" onclick=\"closeIssueModal()\">&#x2715;</button></div><div class=\"modal-body\"><p style=\"font-size:.82rem;color:var(--text-2);margin-bottom:.9rem\">發現藥物資料有誤？請填寫問題描述，按下按鈕即可在 GitHub 建立通報單。</p><div class=\"admin-field\"><label style=\"font-size:.76rem;font-weight:600;color:var(--text-2)\">問題描述 <span style=\"color:#B45309\">*</span></label><textarea class=\"admin-note\" id=\"qIssueDesc\" placeholder=\"例如：Azacitidine 的健保藥價不正確，應為 9,200 元...\" style=\"min-height:90px\"></textarea></div><div class=\"admin-field\" style=\"margin-top:.55rem\"><label style=\"font-size:.76rem;font-weight:600;color:var(--text-2)\">姓名／聯絡方式（選填）</label><input class=\"admin-input\" id=\"qIssueReporter\" type=\"text\" placeholder=\"例如：陳醫師\"></div><div class=\"admin-actions\" style=\"margin-top:.75rem\"><button class=\"secondary-btn\" type=\"button\" onclick=\"submitSimpleIssue(${d.id})\">&#128279; 開啟 GitHub 通報</button><button class=\"ghost-btn\" type=\"button\" onclick=\"closeIssueModal()\">取消</button></div><div class=\"issue-form-note\" style=\"margin-top:.6rem\">點擊後會在新分頁開啟 GitHub 頁面，確認內容後按「Submit new issue」即完成通報。</div></div>`;document.getElementById('issueModal').classList.add('open');}
