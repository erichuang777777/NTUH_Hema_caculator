...
.section-toggle{background:none;border:1px solid var(--border);cursor:pointer;font-size:.7rem;color:var(--text-muted);padding:.15rem .45rem;border-radius:999px;font-family:inherit}
.section-toggle:hover{background:var(--surface-2);color:var(--text)}
...
function openIssueReporter(drugId){const d=findDrugById(_HD,drugId);if(!d) return;const el=document.getElementById('issueC');const h=['<div class="modal-header">','<div class="modal-icon">&#9888;</div>','<div style="flex:1">','<div class="modal-title">問題通報</div>',`<div class=\"modal-subtitle\">${escHtml(d.generic_name)} &middot; ${escHtml(d.trade_names||'')}</div>`,'</div>','<button class="modal-close" type="button" onclick="closeIssueModal()">&#x2715;</button>','</div>','<div class="modal-body">','<p style="font-size:.82rem;color:var(--text-2);margin-bottom:.9rem">發現藥物資料有誤？填寫描述後按下按鈕，即可在 GitHub 建立通報單。</p>','<div class="admin-field">','<label style="font-size:.76rem;font-weight:600;color:var(--text-2)">問題描述 <span style="color:#B45309">*</span></label>','<textarea class="admin-note" id="qIssueDesc" placeholder="例如：Azacitidine 的健保藥價不正確，應為 9,200 元..." style="min-height:90px"></textarea>','</div>','<div class="admin-field" style="margin-top:.55rem">','<label style="font-size:.76rem;font-weight:600;color:var(--text-2)">姓名／聯絡方式（選填）</label>','<input class="admin-input" id="qIssueReporter" type="text" placeholder="例如：陳醫師">','</div>','<div class="admin-actions" style="margin-top:.75rem">',`<button class=\"secondary-btn\" type=\"button\" onclick=\"submitSimpleIssue(${d.id})\">&#128279; 開啟 GitHub 通報</button>`,'<button class="ghost-btn" type="button" onclick="closeIssueModal()">取消</button>','</div>','<div class="issue-form-note" style="margin-top:.6rem">點擊後新分頁開啟 GitHub 頁面，確認內容後按「Submit new issue」即完成通報。</div>','</div>'].join('');el.innerHTML=h;document.getElementById('issueModal').classList.add('open');}
function closeIssueModal(){document.getElementById('issueModal').classList.remove('open');}
function submitSimpleIssue(drugId){const d=findDrugById(_HD,drugId);if(!d) return;const desc=(document.getElementById('qIssueDesc')?.value||'').trim();const reporter=(document.getElementById('qIssueReporter')?.value||'').trim();if(!desc){alert('請填寫問題描述');return;}const title=`[藥物資料] ${d.generic_name}（${d.trade_names||''}）問題通報`;const body=[`**藥物**：${d.generic_name}（${d.trade_names||''}）`,'**問題描述**：',desc,reporter?`**通報者**：${reporter}`:'','---','*此通報由血液腫瘤藥物查詢系統產生*'].filter(Boolean).join('\n\n');if(!ISSUE_NEW_URL||ISSUE_NEW_URL.includes('__')){alert('未設定 GitHub 通報連結');return;}const url=ISSUE_NEW_URL+'?title='+encodeURIComponent(title)+'&body='+encodeURIComponent(body)+'&labels='+encodeURIComponent('drug-data');window.open(url,'_blank','noopener');closeIssueModal();}
...
function toggleSection(id){const el=document.getElementById(id);if(!el) return;const c=el.classList.toggle('is-collapsed');const btn=document.querySelector('[data-toggle="'+id+'"]');if(btn) btn.textContent=c?'展開':'收合';}
