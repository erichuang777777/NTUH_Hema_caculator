...
function showDet(id){
  ...
  const nhiDate=fmtRocDate(primaryEffectiveDate(d))||'115/04/01';
  ...
  if(nfo?.dose_per_bsa){
    ...
    cHtml=`<div class="calc-box"><div class="calc-title">&#128176; 費用試算（每療程周期） <span style="font-size:.7rem;font-weight:400;color:var(--heme-light)">來源：健保藥價基準 115/4/1</span></div><div class="calc-row"><span class="calc-label">BSA</span><input class="calc-input" id="cB${id}" type="number" value="1.7" step="0.1" min="0.5" max="3"><span class="calc-unit">m²</span></div><div class="calc-row"><span class="calc-label">劑量</span><input class="calc-input" id="cD${id}" type="number" value="${nfo.dose_per_bsa}" step="1"><span class="calc-unit">${nfo.unit||'mg/m²'}</span></div><div class="calc-row"><span class="calc-label">天數</span><input class="calc-input" id="cDy${id}" type="number" value="${defDays}" step="1" min="1"><span class="calc-unit">天/周期</span></div><button class="calc-btn" onclick="cBSA(${id},${p},${uMg})">計算</button><div id="cR${id}" class="calc-result" style="display:none"></div></div>`;
  } else if(nfo?.dose_per_kg){
    cHtml=`<div class="calc-box"><div class="calc-title">&#128176; 費用試算（每次給藥） <span style="font-size:.7rem;font-weight:400;color:var(--heme-light)">來源：健保藥價基準 115/4/1</span></div><div class="calc-row"><span class="calc-label">體重</span><input class="calc-input" id="cW${id}" type="number" value="60" step="1"><span class="calc-unit">kg</span></div><div class="calc-row"><span class="calc-label">劑量</span><input class="calc-input" id="cKD${id}" type="number" value="${nfo.dose_per_kg}" step="0.1"><span class="calc-unit">${nfo.unit||'mg/kg'}</span></div><button class="calc-btn" onclick="cKG(${id},${p},${uMg})">計算</button><div id="cR${id}" class="calc-result" style="display:none"></div></div>`;
  } else if(nfo?.dose_fixed){
    const defD=nfo.schedule?parseInt(nfo.schedule):28;
    cHtml=`<div class="calc-box"><div class="calc-title">&#128176; 費用試算（口服療程） <span style="font-size:.7rem;font-weight:400;color:var(--heme-light)">來源：健保藥價基準 115/4/1</span></div><div class="calc-row"><span class="calc-label">每日劑量</span><input class="calc-input" id="cFD${id}" type="number" value="${nfo.dose_fixed}" step="1"><span class="calc-unit">${nfo.unit||'mg'}/日</span></div><div class="calc-row"><span class="calc-label">天數</span><input class="calc-input" id="cFDy${id}" type="number" value="${defD}" step="1" min="1"><span class="calc-unit">天</span></div><button class="calc-btn" onclick="cFix(${id},${p},${uMg})">計算</button><div id="cR${id}" class="calc-result" style="display:none"></div></div>`;
  }
...
function showRes(id,tot,units,p,cost){const el=document.getElementById('cR'+id);if(!el) return;el.style.display='block';el.innerHTML=`總量 ${tot.toFixed(0)} mg &#8594; ${units} 瓶/粒 &#215; NT$${p.toLocaleString()} = <span>NT$${cost.toLocaleString()}</span>`;}
